...
dojo.declare("samiam.store.Service", null, {
	...
	// 		A dictionary mapping dojo store method names to service method names
	methodMapping: null,
	...
	constructor: function(service, options) {
		...
		dojo.forEach(this._storeMethodNames, function(name) {
	...
	_mapMethod: function(name, serviceMethod) {
		...
		if(name == "query") {
			this[name] = function(query, directives) {
				...
				return dojo.store.util.QueryResults(dojo.when(
					serviceMethod.apply(service, args), 
					dojo.hitch(store, "_onQueryResult"),
					dojo.hitch(store, "_onQueryError"),
					dojo.hitch(store, "_onQueryProgress")
				));
			};
